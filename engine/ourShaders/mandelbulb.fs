...
vec3 intersect(vec3 ro, vec3 rd) {
    ...
    for (int i = 0; i < 48; i++) {
        if (error < pixel_size * 0.5 || t > 20.0) {
            // jump out but keep loop structure valid
        } else {
            c = f(ro + rd * t);
            d = c.x;
...
void main() {
    ...
    if (res.x > 0.0) {
        ...
        float sky = 0.6 + 0.4 * max(0.0, dot(n, vec3(0,1,0)));
    ...
    FragColor = vec4(col, smoothstep(0.55, 0.76, 1.0 - res.x / 5.0));
}
