...
float mapScene(vec3 p){
    // --- canopy base hexagon ---
    p.yz*=Rot(3.14);
    float scale=mix(.4,.9,smoothstep(.6,.5,p.y));
    p.xz*=scale;
    
    float hex = sdHexPrism(p, 1., .5);

    // --- rounded top (sphere clamp) ---
    float dome = sdSphere(p - vec3(0, 0.4, 0.0), 0.2);

    // smooth union for roundness
    float canopy = smin(hex, dome,0.2);
    //float canopy=mix(hex,dome,smoothsetep(hex,dome,3.)) ;

    //substract sphere
    float negSphere=sdSphere(p,0.55);
    
    float final=max(-negSphere,canopy);
